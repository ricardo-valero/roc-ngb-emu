...
# per-frame `Buttons` state; it delegates the register read here.
...
Joypad :: [].{
    ...
    p1 = |b, stored| {
        ...
        dpad = if sel.bitwise_and(0x10) == 0x00 { nibble(b.right, b.left, b.up, b.down) } else { 0x0F }
        actions = if sel.bitwise_and(0x20) == 0x00 { nibble(b.a, b.b, b.select, b.start) } else { 0x0F }
    ...
    bit : Bool, U8 -> U8
    bit = |pressed, mask| if pressed { 0x00 } else { mask }
}
