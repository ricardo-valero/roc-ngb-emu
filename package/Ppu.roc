...
Ppu := {
	...
	tick : Ppu, Bus, U64 -> { ppu : Ppu, bus : Bus }
	tick = |ppu0, bus0, cycles| {
		if bus0.read(0xFF40).bitwise_and(0x80) == 0x00 {
			# LCD off: LY pinned to 0, mode 0, nothing advances
			bus1 = bus0.poke(0xFF44, 0x00).poke(0xFF41, bus0.read(0xFF41).bitwise_and(0xFC))
			{ ppu: { ..ppu0, dots: 0, window_line: 0 }, bus: bus1 }
		} else {
			var ppu = ppu0
			var bus = bus0
			...
			while remaining > 0 {
				...
				if ppu.dots == boundary {
					if boundary == 80 {
						...
						bus = set_mode(bus, 3)
						ppu = render_line(ppu, bus, ly)
						{}
					} else if boundary == 252 {
						# enter HBlank (mode 0); an armed HBlank DMA copies here
						bus = Bus.hdma_hblank(stat_interrupt(set_mode(bus, 0), 0x08))
						{}
					} else {
						# line complete
						ppu = { ..ppu, dots: 0 }
						...
						bus = check_lyc(bus.poke(0xFF44, next_ly), next_ly)
						if next_ly == 144 {
							# enter VBlank (mode 1)
							bus = stat_interrupt(set_mode(bus, 1), 0x10).request_interrupt(0)
							{}
						} else if next_ly == 0 {
							# frame start
							ppu = { ..ppu, window_line: 0 }
							bus = stat_interrupt(set_mode(bus, 2), 0x20)
							{}
						} else if next_ly < 144 {
	...
	set_mode : Bus, U8 -> Bus
	set_mode = |bus, mode|
		bus.poke(0xFF41, bus.read(0xFF41).bitwise_and(0xFC).bitwise_or(mode))

	# Raise IF bit 1 when the given STAT enable bit is set
	stat_interrupt : Bus, U8 -> Bus
	stat_interrupt = |bus, enable_mask|
		if bus.read(0xFF41).bitwise_and(enable_mask) != 0x00 {
			bus.request_interrupt(1)
		} else {
			bus
		}

	check_lyc : Bus, U8 -> Bus
	check_lyc = |bus, ly| {
		stat = bus.read(0xFF41)
		if ly == bus.read(0xFF45) {
			with_flag = bus.poke(0xFF41, stat.bitwise_or(0x04))
			stat_interrupt(with_flag, 0x40)
		} else {
			bus.poke(0xFF41, stat.bitwise_and(0xFB))
		}
	}

	# Compose one scanline: background, window, then sprites
	render_line : Ppu, Bus, U8 -> Ppu
	render_line = |ppu, bus, ly|
		if bus.is_cgb() {
			render_line_cgb(ppu, bus, ly)
		} else {
			render_line_dmg(ppu, bus, ly)
		}

	render_line_dmg : Ppu, Bus, U8 -> Ppu
	render_line_dmg = |ppu, bus, ly| {
		lcdc = bus.read(0xFF40)
		bgp = bus.read(0xFF47)
		scx = bus.read(0xFF43).to_u16()
		scy = bus.read(0xFF42).to_u16()
		wx = bus.read(0xFF4B).to_u16()
		wy = bus.read(0xFF4A)
		...
		while x < 160 {
			...
			color =
				if in_window {
					tile_color(bus, lcdc, 0x40, x.plus(7).minus(wx), wl.to_u16())
				} else if bg_enabled {
					tile_color(bus, lcdc, 0x08, x.plus(scx).bitwise_and(0xFF), ly16.plus(scy).bitwise_and(0xFF))
				} else {
					0
				}
			if in_window {
		...
		fb2 =
			if lcdc.bitwise_and(0x02) != 0x00 {
				render_sprites(bus, lcdc, ly16, raw, fb)
			} else {
				fb
			}

		next_window_line = if window_rendered {
			wl.plus_wrap(1)
		} else {
			wl
		}
	...
	tile_color : Bus, U8, U8, U16, U16 -> U8
	tile_color = |bus, lcdc, map_mask, px, py| {
		map_base = if lcdc.bitwise_and(map_mask) != 0x00 {
			0x9C00
		} else {
			0x9800
		}
		tile_index = bus.read(map_base.plus(py.shr_zf_wrap(3).shl_wrap(5)).plus(px.shr_zf_wrap(3)))
		...
		pixel_from_tile_row(bus, tile_addr.plus(row.shl_wrap(1)), px.bitwise_and(0x07))
	}

	# Decode one pixel from a 2bpp tile row at addr; col is 0 (leftmost) to 7
	pixel_from_tile_row : Bus, U16, U16 -> U8
	pixel_from_tile_row = |bus, addr, col| {
		bit = (7.U8).minus(col.to_u8_wrap())
		lo = bus.read(addr).shr_zf_wrap(bit).bitwise_and(0x01)
		hi = bus.read(addr.plus(1)).shr_zf_wrap(bit).bitwise_and(0x01)
	...
	render_sprites : Bus, U8, U16, List(U8), List(U16) -> List(U16)
	render_sprites = |bus, lcdc, ly16, raw, fb0| {
		height = if lcdc.bitwise_and(0x04) != 0x00 {
			16.U16
		} else {
			8.U16
		}
		...
		while x < 160 {
			...
			while j < selected.len() {
				...
				sx = bus.read(base.plus(1)).to_u16()
				if x.plus(8) >= sx and x.plus(8) < sx.plus(8) and sx < best_x {
					color = sprite_color(bus, base, height, x, ly16)
					if color != 0x00 {
						best_x = sx
						best_color = color
						best_attrs = bus.read(base.plus(3))
					} else {
						{}
					}
				} else {
			...
			if best_color != 0x00 {
				...
				if behind_bg and bg_color != 0x00 {
					...
					obp = if best_attrs.bitwise_and(0x10) != 0x00 {
						bus.read(0xFF49)
					} else {
						bus.read(0xFF48)
					}
	...
	render_line_cgb : Ppu, Bus, U8 -> Ppu
	render_line_cgb = |ppu, bus, ly| {
		lcdc = bus.read(0xFF40)
		scx = bus.read(0xFF43).to_u16()
		scy = bus.read(0xFF42).to_u16()
		...
		while x < 160 {
			...
			p =
				if in_window {
					cgb_tile_pixel(bus, lcdc, 0x40, x.plus(7).minus(wx), wl.to_u16())
				} else {
					cgb_tile_pixel(bus, lcdc, 0x08, x.plus(scx).bitwise_and(0xFF), ly16.plus(scy).bitwise_and(0xFF))
				}
			if in_window {
				window_rendered = Bool.True
			} else {
			...
			fb = fb.set(ly16.to_u64().shl_wrap(7).plus(ly16.to_u64().shl_wrap(5)).plus(x.to_u64()), bg_color_555(bus, p.pal, p.color)) ?? fb
			x = x.plus(1)
		}

		fb2 =
			if lcdc.bitwise_and(0x02) != 0x00 {
				render_sprites_cgb(bus, lcdc, ly16, raw, prio, master_bg, fb)
			} else {
				fb
			}
	...
	cgb_tile_pixel : Bus, U8, U8, U16, U16 -> { color : U8, pal : U8, priority : U8 }
	cgb_tile_pixel = |bus, lcdc, map_mask, px, py| {
		map_base = if lcdc.bitwise_and(map_mask) != 0x00 {
			0x9C00
		} else {
			0x9800
		...
		tile_index = Bus.read_vram(bus, 0, entry_addr)
		attr = Bus.read_vram(bus, 1, entry_addr)
		row0 = py.bitwise_and(0x07)
		row = if attr.bitwise_and(0x40) != 0x00 {
			U16.minus(7, row0)
		} else {
			row0
		}
		col0 = px.bitwise_and(0x07)
		col = if attr.bitwise_and(0x20) != 0x00 {
			U16.minus(7, col0)
		} else {
			col0
		}
		...
		color = pixel_from_tile_row_banked(bus, bank, tile_addr.plus(row.shl_wrap(1)), col)
	...
	pixel_from_tile_row_banked : Bus, U8, U16, U16 -> U8
	pixel_from_tile_row_banked = |bus, bank, addr, col| {
		bit = (7.U8).minus(col.to_u8_wrap())
		lo = Bus.read_vram(bus, bank, addr).shr_zf_wrap(bit).bitwise_and(0x01)
		hi = Bus.read_vram(bus, bank, addr.plus(1)).shr_zf_wrap(bit).bitwise_and(0x01)
		hi.shl_wrap(1).bitwise_or(lo)
	}

	# Raw BGR555 from palette RAM: palette n, color c at bytes n*8 + c*2
	# (little-endian, bit 15 unused)
	bg_color_555 : Bus, U8, U8 -> U16
	bg_color_555 = |bus, pal, color| {
		base = pal.to_u64().shl_wrap(3).plus(color.to_u64().shl_wrap(1))
		lo = bus.bg_pal_byte(base).to_u16()
		hi = bus.bg_pal_byte(base.plus(1)).to_u16()
		hi.shl_wrap(8).bitwise_or(lo).bitwise_and(0x7FFF)
	}

	ob_color_555 : Bus, U8, U8 -> U16
	ob_color_555 = |bus, pal, color| {
		base = pal.to_u64().shl_wrap(3).plus(color.to_u64().shl_wrap(1))
		lo = bus.ob_pal_byte(base).to_u16()
		hi = bus.ob_pal_byte(base.plus(1)).to_u16()
		hi.shl_wrap(8).bitwise_or(lo).bitwise_and(0x7FFF)
	}
	...
	render_sprites_cgb : Bus, U8, U16, List(U8), List(U8), Bool, List(U16) -> List(U16)
	render_sprites_cgb = |bus, lcdc, ly16, raw, prio, master_bg, fb0| {
		height = if lcdc.bitwise_and(0x04) != 0x00 {
			16.U16
		} else {
			8.U16
		...
		while x < 160 {
			...
			while j < selected.len() {
				...
				candidate = if x_order {
					covers and sx < best_x
				} else {
					covers and found == Bool.False
				}
				if candidate {
					color = sprite_color_cgb(bus, base, height, x, ly16)
					if color != 0x00 {
						best_x = sx
						best_color = color
						best_attrs = bus.read(base.plus(3))
			...
			if best_color != 0x00 {
				...
				if hidden {
					...
					idx = ly16.to_u64().shl_wrap(7).plus(ly16.to_u64().shl_wrap(5)).plus(x.to_u64())
					fb = fb.set(idx, ob_color_555(bus, best_attrs.bitwise_and(0x07), best_color)) ?? fb
				}
			} else {
				{}
			}
	...
	sprite_color_cgb : Bus, U16, U16, U16, U16 -> U8
	sprite_color_cgb = |bus, base, height, x, ly16| {
		sy = bus.read(base).to_u16()
		sx = bus.read(base.plus(1)).to_u16()
		tile = bus.read(base.plus(2))
		attrs = bus.read(base.plus(3))
		row0 = ly16.plus(16).minus(sy)
		row = if attrs.bitwise_and(0x40) != 0x00 {
			height.minus(1).minus(row0)
		} else {
			row0
		}
		col0 = x.plus(8).minus(sx)
		col = if attrs.bitwise_and(0x20) != 0x00 {
			U16.minus(7, col0)
		} else {
			col0
		}
		tile_index = if height == 16 {
			tile.bitwise_and(0xFE)
		} else {
			tile
		}
	...
	debug_background : Bus -> List(U16)
	debug_background = |bus| {
		lcdc = bus.read(0xFF40)
		bgp = bus.read(0xFF47)
		cgb = bus.is_cgb()
		...
		while y < 256 {
			...
			while x < 256 {
				px =
					if cgb {
						p = cgb_tile_pixel(bus, lcdc, 0x08, x, y)
						bg_color_555(bus, p.pal, p.color)
					} else {
						gray_555(palette_shade(bgp, tile_color(bus, lcdc, 0x08, x, y)))
					}
		...
		while t < 384 {
			...
			while py < 8 {
				...
				while px < 8 {
					...
					out = out.set(idx, gray_555(pixel_from_tile_row(bus, addr, px))) ?? out
					px = px.plus(1)
				}
				py = py.plus(1)
			}
	...
	oam_cell_pixel : Bus, U16, U16, U16, U16 -> U16
	oam_cell_pixel = |bus, base, height, col0, row0| {
		attrs = bus.read(base.plus(3))
		row = if attrs.bitwise_and(0x40) != 0x00 {
			height.minus(1).minus(row0)
		} else {
		...
		bank = if bus.is_cgb() {
			attrs.bitwise_and(0x08).shr_zf_wrap(3)
		} else {
			0
		}
		addr = U16.plus(0x8000, tile_index.to_u16().shl_wrap(4)).plus(row.shl_wrap(1))
		color = pixel_from_tile_row_banked(bus, bank, addr, col)
		if color == 0x00 {
			gray_555(0)
		} else if bus.is_cgb() {
			ob_color_555(bus, attrs.bitwise_and(0x07), color)
		} else {
			obp = if attrs.bitwise_and(0x10) != 0x00 {
				bus.read(0xFF49)
			} else {
				bus.read(0xFF48)
			}
	...
	debug_oam : Bus -> List(U16)
	debug_oam = |bus| {
		height = if bus.read(0xFF40).bitwise_and(0x04) != 0x00 {
			16.U16
		} else {
			8.U16
		}
		...
		while i < 40 {
			...
			while row < height {
				...
				while col < 8 {
					...
					out = out.set(idx, oam_cell_pixel(bus, base, height, col, row)) ?? out
					col = col.plus(1)
				}
				row = row.plus(1)
			}
	...
	sprite_color : Bus, U16, U16, U16, U16 -> U8
	sprite_color = |bus, base, height, x, ly16| {
		sy = bus.read(base).to_u16()
		sx = bus.read(base.plus(1)).to_u16()
		tile = bus.read(base.plus(2))
		attrs = bus.read(base.plus(3))
...
fresh : {} -> { ppu : Ppu, bus : Bus }
fresh = |_| { ppu: Ppu.init({}), bus: Bus.init(test_rom).poke(0xFF0F, 0x00) }

# Mode transitions within a visible line
expect {
	f = fresh({})
	r = f.ppu.tick(f.bus, 81)
	r.bus.read(0xFF41).bitwise_and(0x03) == 3
}
expect {
	f = fresh({})
	r = f.ppu.tick(f.bus, 253)
	r.bus.read(0xFF41).bitwise_and(0x03) == 0
}

# Line advance and VBlank entry
expect {
	f = fresh({})
	r = f.ppu.tick(f.bus, 456)
	r.bus.read(0xFF44) == 1 and r.bus.read(0xFF41).bitwise_and(0x03) == 2
}
expect {
	f = fresh({})
	r = f.ppu.tick(f.bus, 456 * 144)
	r.bus.read(0xFF44) == 144
		and r.bus.read(0xFF41).bitwise_and(0x03) == 1
			and r.bus.read(0xFF0F).bitwise_and(0x01) == 0x01
}

# Full frame wraps LY back to the starting line
expect {
	f = fresh({})
	r = f.ppu.tick(f.bus, 70224)
	r.bus.read(0xFF44) == 0
}
...
expect {
	f = fresh({})
	a = f.ppu.tick(f.bus, 456 * 144)
	b = a.ppu.tick(a.bus.poke(0xFF0F, 0x00), 456 * 10)
	b.bus.read(0xFF0F).bitwise_and(0x01) == 0x00
}

# LYC coincidence sets STAT bit 2 and raises STAT interrupt when enabled
expect {
	f = fresh({})
	armed = f.bus.poke(0xFF45, 40).poke(0xFF41, f.bus.read(0xFF41).bitwise_or(0x40))
	r = f.ppu.tick(armed, 456 * 40)
	r.bus.read(0xFF44) == 40
		and r.bus.read(0xFF41).bitwise_and(0x04) == 0x04
			and r.bus.read(0xFF0F).bitwise_and(0x02) == 0x02
}

# LCD off pins LY to 0
expect {
	f = fresh({})
	off = f.bus.poke(0xFF40, 0x00)
...
paint_tile : Bus, U16 -> Bus
paint_tile = |bus0, addr| {
	var bus = bus0
	var i = 0.U16
	while i < 16 {
		bus = bus.poke(addr.plus(i), 0xFF)
		i = i.plus(1)
	}
	bus
}
...
expect {
	f = fresh({})
	m = paint_tile(f.bus, 0x8010).poke(0x9800, 0x01).write(0xFF47, 0xE4)
	r = f.ppu.tick(m, 81)
...
expect {
	f = fresh({})
	m = paint_tile(f.bus, 0x8010).poke(0x981F, 0x01).write(0xFF47, 0xE4).write(0xFF43, 252)
	r = f.ppu.tick(m, 81)
...
expect {
	f = fresh({})
	m = paint_tile(f.bus, 0x8010).poke(0x9800, 0x01).write(0xFF47, 0xE4).write(0xFF40, 0x90)
	r = f.ppu.tick(m, 81)
...
expect {
	f = fresh({})
	var m = paint_tile(f.bus, 0x8010).write(0xFF47, 0x00).write(0xFF48, 0xE4).write(0xFF40, 0x93)
	var i = 0.U16
	while i < 11 {
		base = U16.plus(0xFE00, i.shl_wrap(2))
...
expect {
	f = fresh({})
	m = paint_tile(f.bus, 0x8030) # tile 3: lower half of the 8x16 pair 2/3
...
expect {
	f = fresh({})
	m = paint_tile(f.bus, 0x8010).poke(0x9800, 0x01).poke(0x9821, 0x01).write(0xFF47, 0xE4)
	r = f.ppu.tick(m, 70224)
	map = Ppu.debug_background(r.bus)
	var ok = Bool.True
	var y = 0.U64
	while y < 144 {
		var x = 0.U64
