...
Ppu := {
        ...
        while x < 160 {
            ...
            if best_color != 0x00 {
                ...
                if behind_bg and bg_color != 0x00 {
                    ...
                    obp = if best_attrs.bitwise_and(0x10) != 0x00 { bus.read(0xFF49) } else { bus.read(0xFF48) }
    ...
    oam_cell_pixel = |bus, base, height, col0, row0| {
        ...
        tile = bus.read(base.plus(2))
        tile_index = if height == 16 { tile.bitwise_and(0xFE) } else { tile }
        bank = if bus.is_cgb() { attrs.bitwise_and(0x08).shr_zf_wrap(3) } else { 0 }
        addr = U16.plus(0x8000, tile_index.to_u16().shl_wrap(4)).plus(row.shl_wrap(1))
        ...
        if color == 0x00 {
            ...
            obp = if attrs.bitwise_and(0x10) != 0x00 { bus.read(0xFF49) } else { bus.read(0xFF48) }
    ...
    debug_oam = |bus| {
        height = if bus.read(0xFF40).bitwise_and(0x04) != 0x00 { 16.U16 } else { 8.U16 }
