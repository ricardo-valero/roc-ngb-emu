...
Cpu := {
	...
	step = |cpu, bus| {
		...
		if cpu.ime and pending != 0x00 {
			match dispatch(to_st(cpu, bus), pending) {
				(st, cycles) => of_st(st, cycles)
			}
		} else if cpu.halted {
			if pending != 0x00 {
				# Wake regardless of IME; without IME no dispatch happens
				match run_bare(to_st({ ..cpu, halted: Bool.False }, bus)) {
					(st, cycles) => of_st(st, cycles)
				}
			} else {
