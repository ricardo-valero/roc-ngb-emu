...
GameBoy := {
	...
	with_battery = |gb, bytes|
		if Header.has_battery(gb.bus.cart.rom) {
			...
			cart2 = if footer.len() >= 44 {
				cart1.load_battery_footer(footer)
			} else {
				cart1
			}
	...
	run_frame = |gb0, input| match gb0.run_until(input) {
		(gb, _) => gb
	}
	...
	finish = |gb, cycles| {
		video_cycles = if gb.bus.double_speed {
			cycles // 2
		} else {
			cycles
		}
...
after_step = |gb| match gb.step() {
	(g, _) => g
}

cycles_of : GameBoy -> U64
cycles_of = |gb| match gb.step() {
	(_, c) => c
}
...
expect {
	...
	g = match GameBoy.from_raw(s).step_instruction() {
		(gb, _) => gb
	}
