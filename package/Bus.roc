...
Bus := {
	...
	init = |rom| {
		...
		base = {
			...
			model: if cgb_flag == 0x80 or cgb_flag == 0xC0 {
				Cgb
			} else {
				Dmg
			},
	...
	read_cgb_reg = |bus, addr|
		match addr {
			0xFF4D => {
				speed = if bus.double_speed {
					0x80.U8
				} else {
					0x00
				}
				prepare = if bus.key1_prepare {
					0x01.U8
				} else {
					0x00
				}
			...
			0xFF55 => if bus.hdma_active {
				bus.hdma_blocks.minus_wrap(1).bitwise_and(0x7F)
			} else {
				0xFF
			}
	...
	write_bcpd = |bus, value| {
		...
		next = if bus.bcps.bitwise_and(0x80) != 0x00 {
			idx.plus(1).bitwise_and(0x3F).bitwise_or(0x80)
		} else {
			bus.bcps
		}
	...
	write_ocpd = |bus, value| {
		...
		next = if bus.ocps.bitwise_and(0x80) != 0x00 {
			idx.plus(1).bitwise_and(0x3F).bitwise_or(0x80)
		} else {
			bus.ocps
		}
	...
	write = |bus, addr, value|
		if bus.layout == Flat {
			...
			if value.bitwise_and(0x80) == 0x00 {
				...
				if was_off {
					{ ..m, apu_events: m.apu_events.append(0xF1) }
				} else {
					m
				}
			}
