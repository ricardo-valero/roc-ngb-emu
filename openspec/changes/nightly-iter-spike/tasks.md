...
- [ ] 2.3 Convert `Apu.apply_events`'s index loop to iteration over the
      drained event list; package expects gate it
- [ ] 2.4 Convert the cold sites from the 2.1 inventory that read better
      ...
      (design D3)
- [ ] 2.5 Tier 1 green after each conversion; Tier 2 suites re-run at
      the end if available
- [ ] 2.6 Hot paths only if Blargg runs on the branch: convert ONE
      candidate (e.g. the OAM scan), compare against the 1.5 branch
      baseline, keep or revert; record the number either way

## 3. Pure-push survey

- [ ] 3.1 Survey the latest `Builtin.roc` for push-shaped structures
      (queue, deque, ring, sink/`from_iter` targets); record findings
- [ ] 3.2 Write the pure-push section of `report.md`: the
...
- [ ] 3.3 Only if 3.2 recommends it: prototype the module with package
...
- [ ] 4.1 Finish `report.md`: nightly verdict, conversions landed vs
...
- [ ] 4.2 Reduce any compiler bugs found to standalone repros without
      emulator provenance (design D6)
- [ ] 4.3 Commit the branch; leave `main` untouched
