...
- [ ] 1.1 `git mv package/Mmu.roc package/Bus.roc`; rename the `Mmu` nominal
...
- [ ] 1.2 `roc check package/main.roc` and `roc test package/main.roc` pass
      on the rename alone; commit this stage separately

## 2. Extract Timer

- [ ] 2.1 Create `package/Timer.roc` per design D2: counters record + pure
...
- [ ] 2.4 `roc test package/main.roc` passes; import `Timer` from `main.roc`

## 3. Extract Joypad

- [ ] 3.1 Create `package/Joypad.roc` per design D3: `Buttons` type,
      `none`, `p1` (select bits + active-low nibble math ported verbatim)
- [ ] 3.2 `Bus` keeps `buttons : Joypad.Buttons`, delegates its P1 read;
      switch `no_buttons` callers (`GameBoy.no_input`, expects) to
      `Joypad.none`; keep a `Bus.no_buttons` delegator only if a frontend
      needs it
- [ ] 3.3 Move/keep the joypad expects (group select, AND of both groups,
      none-selected reads 0xF) and add direct `Joypad.p1` unit expects
- [ ] 3.4 `roc test package/main.roc` passes; import `Joypad` from `main.roc`

## 4. Verify

- [ ] 4.1 `roc check package/main.roc` and `roc test package/main.roc` pass
      (full suite)
- [ ] 4.2 `roc check/run.roc -- check/mooneye/passlist` passes (timer/halt
      acceptance subset)
- [ ] 4.3 `roc check/run.roc -- check/blargg/passlist` passes
      (`instr_timing`, `dmg_sound` exercise the timer paths)
- [ ] 4.4 `roc check/battery/main.roc` and `roc check/acid2/main.roc` pass;
