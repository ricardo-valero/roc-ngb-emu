...
- [ ] 1.1 Confirm `extract-bus-peripherals` has landed (module names
      `Bus.roc` / `Timer.roc` / `Joypad.roc` exist); if not, pause and
      resequence
- [ ] 1.2 Add the "Hardware map" section to `README.md` per the proposal:
...
- [ ] 1.3 Cross-check every module named in the table against the actual
      `package/` tree and every physical claim against Pan Docs (SoC die
      contents, external SRAM, cartridge PCB)

## 2. Verify

- [ ] 2.1 Table renders correctly (GitHub Markdown preview) and all file
